...
int main(){
    ...
    std::vector<int> start_node({0, 0, 0, 0, 0});
    path_nodes.push(start_node);

    int min_cost = INT_MAX;
    while(!path_nodes.empty()){
        std::vector<int> curr_node = path_nodes.top();
        if((curr_node[1] == (num_rows - 1)) && (curr_node[2] == (num_cols - 1))){
            min_cost = curr_node[0];
            break;
        }
        path_nodes.pop();
        ...
        for(int turn : std::vector<int>({-1, 0, 1})){
            ...
            if((next_steps <= 3) && ((next_row >= 0) && (next_row < num_rows)) && ((next_col >= 0) && (next_col <  num_cols))){
